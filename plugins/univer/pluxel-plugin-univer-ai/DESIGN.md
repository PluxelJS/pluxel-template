...
> 核心原则：后端不直接改 snapshot；前端只用 Univer command/Facade 写入（符合 `plugins/univer/MVP.md` 的硬约束）。
...
实现约定（当前代码已支持）：
- Univer UI 默认发送 `format: 'toon'`（所见即所得、wire 更小）；其它调用方也可发送 `format: 'json'`（便于调试/复用）。
- 若发送 `format: 'json'`，后端会统一 `formatStructured(..., { format: 'toon' })` 注入 prompt；若 JSON parse 失败，会返回带原因的错误。
